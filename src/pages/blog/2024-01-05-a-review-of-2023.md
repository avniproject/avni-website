...
## Impact numbers
A quick view of the impact numbers
...
- Upgrade of libraries in the Android application. We had not performed this activity for about four years, and this was the last responsible time to perform the upgrade.
- Moving of organisations from the old style. We had moved to using the app designer to using the new rules in 2020. Migrating existing organisations to this new format required time and effort. At this point, there are only two more organisations that have to move to the new format. 
- Using ETL for reports. With data that keeps getting added into the Avni database, even simple reports started getting slower. The ETL application was an experiment we started in March 2022 to make reports faster. It works by moving data of each organisation into its own schema, and structuring the data that is easier to use for reports. However, the only way this would work was if we rewrite all reports that we had written until now to use the new format. There were around 1500 reports that need to be migrated. Since 2022, we have been trying to move many reports. 
- Automation. An increase in the need to monitor several details of the application, significant time has been spent on automation activities. Tests, apk generation and deployment have been the primary areas of deployment in the past year.

The Avni team spent approximately one third of its time stabilizing the application. We understand many of these changes have led to disruption on the field and slow-down of development of new features and hope that the changes in 2022 will allow us to move faster this year.
...
- [Mobile Creches](https://www.mobilecreches.org) works with children in construction sites and slum settlements. The organisation runs Balwadis for children below three years, creches for children below five and after-school/bridge courses for children above five. Their programs help improve health, nutrition and early learning of children. Avni is used to maintain attendance, monitor health indicators and track interventions such as special diets. 
- Teach Nagaland is the Teach project implemented for the state of Nagaland. In this project, government school teachers are observed regularly with the aim of improving overall teaching effectiveness in schools. A pilot was performed in late 2023 in the state. The state department will now take up implementation across all government schools in Nagaland. 
- [Bhansali Trust](https://bhansalitrust.in/) is trying out Avni to record data from eye camps
- [IPH India](https://iphindia.org/) started using Avni for multiple programs in the clinical domain
- We are currently working with [Sukhibhava Foundation] (https://sukhibhava.homerun.co/) to use Avni to monitor school sessions on menstrual health and hygiene.
...
- Dedicated hosting for the bigger organisations to reduce the churn of releases
- Dedicated support for the bigger organisations (including L1 support) for better response times for everyone
- Automation with an intent to reduce onboarding costs and streamline support
