...
- **Connected tools (MCP).** The agent reaches GitHub (issues, pull requests, project boards), Google Workspace (docs and sheets), and a real browser through Playwright for exercising the web console during testing.
- **Supporting pieces.** A local mirror of every open issue for fast triage; scripts that check a release is genuinely ready to ship; a knowledge base of past setups that seeds new configuration; and, for field apps that must run offline, small vision and language models running directly on the device.
