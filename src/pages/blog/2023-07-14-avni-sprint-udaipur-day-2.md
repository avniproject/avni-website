...
1. An introduction to Social Bytes NGO and a overview of their Convene Platform
2. An overview of process adopted by "Agency Fund" to vet future investment opportunities
...
Few of their notable achievements have been with following initiatives:
 - CRY Meal system : "Provide underprivileged children access to proper food and nutrition"
     - Has 90+ forms
     - Used Data Visualization using BI tools
     - Integration with Active Directory
 - Cuddles Foundation : "Help underprivileged children recover from Cancer by provide proper nutrition"
   Their approach to solve this problem is as follows
     - Nutritionist uses platform to specify the meal-plan for the child
     - Meal-plan is then used to translate into Groceries list needed for a month
     - The Groceries list is sent to Kirana shop that has a tie-up with the organisation
     - Parent of the child then collects the Groceries from the Kirana shop by specifying an OTP sent by the organisation
     - Earlier the OTP was sent via SMS, the organisation is now switching to Glific for their out-reach and engagement
     
 He then wrapped up his talk by listing out in brief a few other projects, which included few of the following:
1. Svetana
2. Mera Aspathal
3. HelpAge

After this, Girish, another Co-founder from [Social Bytes](https://thesocialbytes.com/) gave an overview of their platform. "Convene" platform, is a collection of libraries that provides following capabilities:
 - Data collectionwith skip-logic, reports and Dashboards
...
Robert, from [The Agency Fund](https://www.agency.fund/), gave us insights into the Data-driven approach they take in evaluating the impact of a project. These insights are then fed into their decision-making process to determine the projects to provide funding for.
...
We returned to the Board room of the hotel to resume Avni Knowledge sharing sessions.

Vinay gave us an overview of Avni models module, which is currently shared across 3 other components : Webapp, Rules-server and client. He explained in-detail its usage while defining rules for an implementation.  
...
One of the critical components of Avni application is the Sync process between the Backend server and the Client application. Since we have had a few new joinees to the Avni team, it was critical for them to have an understanding of how the process works. This would aid in debugging Sync related issues reported by the user and also help while performing data modifications related to sync for an implementation. Major topics covered as part of this walkthrough by me were as follows:
 - Pull Mechanism employed by Avni-client to sync data
 - SyncDetails API call
 - Different types of data synced to the client
...
### Challenges with Primary Healthcare in remote villages of India
 ...
 - Absentee Head of Household, resulting in women / children felling apprehensive and delaying / not availing healthcare
...
Aim is to provide, good quality preventive, promotive and curative care with Humility in "Amrit" clinics. These clinics are setup on land leased out to BHS by the community, to ensure they have significant stake in the implementation of the program. Nurses are the primary work-horses in the clinic. These Nurses are qualified and receive additional training on how to handle major PHC scenarios that occur in those regions. Also, these nurses are usually native to that region and are able to better adjust to the vagaries of life in remote village and also empathise with the local population. They are also equipped with diagnostic tools and kits to aid in quick prognosis of an ailment and have protocols defined to handle deterioration in the patient's condition.
...
![](/img/2023-07-14-avni-sprint-udaipur-day-2/ajeevika-field-visit.jpg)<pre>Avni team-members Sachin(4th from right), Vedant(3rd from right) and Nupoor(2nd from right) in the middle with the "Labour line" support team</pre>
